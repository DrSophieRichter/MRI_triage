...
jhu_fa <- read.csv("C:/Users/sophie/Documents/Documents/Medicine/PhD/Trajectory_paper/Raw_data/JHU_updated/fa_jhu_all_20220203.csv", na.strings = c("", " ", "NA")) %>% select(-X)
...
#merge with data
graded <- adam %>% select(Master_subject_ID, AG_stage, F_grade, Burden_visual)
...
label(data$BS_visual) <- "Brainstem injury"
label(data$Burden_visual) <- "TAI burden (no. of regions)"
...
#Come here: elimate space between text and plot
p3b
...
for (i in 1:length(proteins)){
  ...
  mycut <- optimal.cutpoints(Myprotein ~ BS_visual, data = cutdata, 
                                 ...
                                 tag.healthy = 0, 
                                 direction = "<",
                           control = control.cutpoints(valueSe = 0.9 ))
...
save_as_docx(ft, path = "Figures/Savings.docx")
