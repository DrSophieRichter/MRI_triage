...
####################################################
#
# Adjust biomarkers for time of sampling
#
###################################################

adjust_bio <- function(df, imp, imp_list, mybio){
  ...
  delta_df <- as.data.frame(matrix (, 
                                    nrow = nrow(imp_list[[i]]), 
                                    ncol = length(imp_list)))
  for (i in 1:length(imp_list)){
    ...
    delta_df[,i]   <- bio_2h - bio_pred
    
  }
  
  #pool delta results across multiply imputed datasets
  delta_df$Delta_mean <- rowMeans(delta_df)
  
  ##then add this delta (which may be negative for late samples)
  # to the observed concentration to obtain the time adjusted
  # biomarker result
  # df is the unimputed data
  newname      <- gsub("raw", "", mybio)
  df[,newname] <- df[, paste0(mybio)] + delta_df$Delta_mean
